...
export default function Home() {
  ...
  const getMockResult = () => {
    if (email.endsWith("@test.com")) return { eligible: true, plan: "premium", reason: "" };
    return { eligible: false, plan: "basic", reason: "Not eligible for this quarter." };
  };

  const resetForm = () => {
    setQuestions(["", "", ""]); setGoal(""); setDocLink(""); setConfirmed(false); setSubmitted(false);
  ...
  const checkEligibility = async () => {
    try {
      setLoading(true); setResult(null); setError(null); resetForm();
  ...
  const handleSubmit = async () => {
    ...
    const params = new URLSearchParams({
      email, field, language, mentor,
      question_1: questions[0], question_2: questions[1], question_3: questions[2],
      goal, document_link: docLink, plan: result?.plan || "",
    });
    try {
      await fetch(ZAPIER_WEBHOOK, { method: "POST", body: params });
    } catch (_) {} finally { setSubmitting(false); setSubmitted(true); }
  };
  ...
  const availableMentors = language ? mentorsForField.filter((m) => m.languages.includes(language)) : [];
  ...
  const canSubmit = field && language && mentor && questionsValid && goal.trim() && docLink.trim() && confirmed && !submitting;
    ...
    <div className="min-h-screen bg-[#F7F6F3] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg">
        <div className="mb-10 text-center">
          ...
          <p className="mt-3 text-gray-500 text-sm leading-relaxed">Check your eligibility and submit a session request to your mentor.</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-5">
          {!submitted ? (
            <>
              ...
              <button
                onClick={checkEligibility} disabled={loading || !isValidEmail}
                className="w-full py-3 px-6 rounded-xl bg-[#7c16ff] text-white text-sm font-medium transition-all active:scale-[0.98] disabled:opacity-40"
              >
              ...
              {result && result.eligible && (
                <div className="space-y-5 pt-2">
                  <AlertBox type="success">
                    ...
                    <p className="mt-0.5 text-xs">Your <span className="font-semibold capitalize">{result.plan}</span> plan gives you access to <span className="font-semibold">{SESSION_INFO[result.plan]}</span>.</p>
                  </AlertBox>
                  ...
                  <div className="border-t border-gray-100 pt-5 space-y-5">
                    ...
                    {bothPrefilled ? (
                      ...
                      <>
                        <div>
                          ...
                          <select value={field} onChange={(e) => { setField(e.target.value); setLanguage(""); setMentor(""); }} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm">
                        ...
                        <div>
                          ...
                          <select value={language} onChange={(e) => { setLanguage(e.target.value); setMentor(""); }} disabled={!field} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm disabled:opacity-50">
                        ...
                        <div>
                          ...
                          <select value={mentor} onChange={(e) => setMentor(e.target.value)} disabled={!language} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm disabled:opacity-50">
                            <option value="">{language ? "Choose a mentor…" : "Select a language first…"}</option>
                            {availableMentors.map((m) => <option key={m.name} value={m.name}>{m.name}</option>)}
                          </select>
                        </div>
                      </>
                    )}
                    ...
                    <div className="border-t border-gray-100 pt-5 space-y-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Your top 3 questions*</label>
                        <div className="space-y-2">
                          {questions.map((q, i) => (
                            <div key={i} className="flex items-start gap-2">
                              <span className="mt-3 text-xs font-medium text-gray-400 w-4">{i + 1}.</span>
                              <input type="text" value={q} onChange={(e) => { const updated = [...questions]; updated[i] = e.target.value; setQuestions(updated); }} placeholder={`Question ${i + 1}`} className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm" />
                            </div>
                          ))}
                        </div>
                      </div>
                      <textarea value={goal} onChange={(e) => setGoal(e.target.value)} placeholder="Main goal for the session*" rows={3} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm" />
                    </div>

                    <div className="border-t border-gray-100 pt-5 space-y-3">
                      <SectionTitle number="4" title="Supporting documents" />
                      {docHint && <div className="p-3 rounded-lg bg-blue-50 text-xs text-blue-700 border border-blue-100">{docHint}</div>}
                      <div>
                        <div className="flex justify-between items-end mb-1.5">
                          <label className="text-sm font-medium text-gray-700">Document link*</label>
                          <a href="https://drive.google.com/drive/my-drive" target="_blank" rel="noopener noreferrer" className="text-[11px] font-medium text-[#7c16ff] hover:underline flex items-center gap-1">Upload to Google Drive</a>
                        </div>
                        <input type="text" value={docLink} onChange={(e) => setDocLink(e.target.value)} placeholder="Paste shared link here..." className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm" />
                      </div>
                    </div>

                    <div className="border-t border-gray-100 pt-5">
                      <label className="flex items-start gap-3 cursor-pointer group">
                        <input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} className="mt-0.5 h-4 w-4 accent-black" />
                        <span className="text-sm text-gray-600">I confirm that I have read the <a href="https://docs.google.com/document/d/1UnzhvBGZDzefqhHtCNKArOByVAHwfHxfYd5_3NEPc0k/preview" target="_blank" className="underline font-medium text-gray-900">Mentee Agreement</a></span>
                      </label>
                    </div>

                    <button onClick={handleSubmit} disabled={!canSubmit} className="w-full py-3 px-6 rounded-xl bg-[#7c16ff] text-white text-sm font-medium transition-all active:scale-[0.98] disabled:opacity-40">{submitting ? <Spinner /> : "Submit request"}</button>
                  </div>
                </div>
              )}
            </>
          ) : (
            <AlertBox type="success">Your request has been submitted. The mentor will review it within 48 hours.</AlertBox>
          )}
        </div>
        <p className="text-center text-xs text-gray-400 mt-6">Have questions? Reach out to <a href="mailto:mentorship@takadao.io" className="underline hover:text-gray-600 transition-colors">mentorship@takadao.io</a></p>
      </div>
    </div>
  );
}
...
function SectionTitle({ number, title }: { number: string | number; title: string }) {
  return (
    <div className="flex items-center gap-2.5">
      <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#7c16ff] text-white text-xs font-semibold">{number}</span>
...
function AlertBox({ type, children }: { type: "error" | "warning" | "success" | "info"; children: React.ReactNode }) {
  const styles = { error: "bg-red-50 border-red-100 text-red-700", warning: "bg-amber-50 border-amber-100 text-amber-800", success: "bg-emerald-50 border-emerald-100 text-emerald-800", info: "bg-blue-50 border-blue-100 text-blue-800" };
  return (
    <div className={`flex items-start gap-3 p-4 rounded-xl border text-sm ${styles[type]}`}>
      <svg className="w-4 h-4 mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
      <div>{children}</div>
    </div>
  );
}
...
function Spinner() {
  return <div className="flex items-center gap-2"><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8v8z"/></svg>Loading…</div>;
}